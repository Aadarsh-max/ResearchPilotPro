...
const TopicInput = ({ onSearch }) => {
  ...
  const isValidTopic = (text) => {
    ...
    // Must contain alphabets (not only symbols or numbers)
  ...
  const handleSubmit = (e) => {
    ...
    if (!isValidTopic(topic)) {
      setError(
        "Enter a valid research topic (e.g., AI in Healthcare, Edge Computing)",
      );
      return;
    }
  ...
  return (
    <div className="w-full flex justify-center relative">
      <div
        className="w-full max-w-4xl bg-white p-10 rounded-3xl border border-gray-100
